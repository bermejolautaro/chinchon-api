...
using static Chinchon.API.Result<Chinchon.API.Dtos.PlayerStateDto>;
...
namespace Chinchon.API
{
    public class GameFileSystemRepository : IGameRepository
    {
        ...
        public async Task<Result<PlayerStateDto>> GetPlayer(Guid gameGuid, Guid playerGuid)
        {
            GameState? gameState = null;
            ExtendedGameState? extendedState = null;

            try
            {
                gameState = await GetGame(_path, gameGuid);
                extendedState = await GetExtendedGameState(_path, gameGuid);
            }
            catch(Exception e)
            {
            ...
            {
                return Error("A player with this Id does not exist");
            }

            return Ok(new PlayerStateDto()
            {
                Id = player.Id,
                Points = player.Points,
                Cards = player.Cards,
                IsPlayerTurn = player.Id == gameState.PlayerTurn,
                TopCardInPile = gameState.Pile.FirstOrDefault()
            });
        }
        ...
        public async Task<Result<PlayerStateDto>> PickCardFromDeck(Guid gameGuid, Guid playerGuid)
        {
            ...
            {
                return Error("Only can play when is your turn");
            }
                ...
                case SuccessResult success:
                    {
                        ...
                        {
                            return Error("Error trying to save state");
                        }
                        ...
                        return Ok(new PlayerStateDto()
                        {
                            ...
                            TopCardInPile = success.GameState.Pile.FirstOrDefault()
                        });
                    }
                case ErrorResult error:
                    {
        ...
        public async Task<Result<PlayerStateDto>> PickCardFromPile(Guid gameGuid, Guid playerGuid)
        {
            var gameState = await GetGame(_path, gameGuid);
            var extendedState = await GetExtendedGameState(_path, gameGuid);
        ...
        public async Task<Result<PlayerStateDto>> DiscardCard(Guid gameGuid, Guid playerGuid, Card card)
        {
            var gameState = await GetGame(_path, gameGuid);
            var extendedState = await GetExtendedGameState(_path, gameGuid);
                ...
                case SuccessResult success:
                    {
                        ...
                        {
                            return Error("Trying to save state");
                        }
        ...
        public async Task<Result<PlayerStateDto>> Cut(Guid gameGuid, Guid playerGuid, Group? firstGroup, Group? secondGroup, Card? cardToCutWith)
        {
            var gameState = await GetGame(_path, gameGuid);
            var extendedState = await GetExtendedGameState(_path, gameGuid);
